...
interface FetchOrdersActivity {
    fun fetchOrders(organizationId: UUID, orderIds: List<UUID>): List<Order>
}

@ActivityInterface
interface FetchVehiclesActivity {
    fun fetchVehicles(organizationId: UUID, vehicleIds: List<UUID>): List<com.vrp.domain.model.Vehicle>
}
...
interface PersistSolutionActivity {
    fun persist(jobId: UUID, organizationId: UUID, result: SolverResult)
}
